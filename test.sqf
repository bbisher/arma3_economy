_is_affordable = {
	params["_cost", "_resources"];
	private _return = False;
	_cost = _this select 0;
	_resources = _this select 1;

	private _funds = _resources select 0;
	private _ammo = _resources select 1;
	private _fuel = _resources select 2;

	private _cost_funds = _cost select 0;
	private _cost_ammo = _cost select 1;
	private _cost_fuel = _cost select 2;

	if((_cost_funds <= _funds) && (_cost_ammo <= _ammo) && (_cost_fuel <= _fuel)) then{ //Replace with this later.
		_return = True;
	} else {
		_return = False;
	};
...
_cost = [10,10,10];
_resources = [11,11,11];
_results = [_cost, _resources] call _is_affordable;
hint str _results;
